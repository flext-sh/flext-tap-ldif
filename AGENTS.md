...
**Package:** `flext_tap_ldif` · deps: `flext-core`, `flext-ldif`, `flext-meltano`,
`flext-observability`

## Overview

Singer **tap** (extractor) for LDIF-format data. Thin driver over `flext-meltano`
(ADR-006), delegating parsing to `flext-ldif`.
...
- The **active** entrypoint is `tap.py` (not a `.bak`). LDIF parsing is delegated to
  `flext-ldif`'s registry.
