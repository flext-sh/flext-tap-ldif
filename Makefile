...
ldif-validate: ## Validate LDIF file format
	PYTHONPATH=$(SRC_DIR) $(POETRY) run python -c "from flext_tap_ldif.ldif_processor import validate_ldif; validate_ldif()"

ldif-parse: ## Test LDIF parsing functionality
	PYTHONPATH=$(SRC_DIR) $(POETRY) run python -c "from flext_tap_ldif.ldif_processor import test_parsing; test_parsing()"

ldif-test: ## Run comprehensive LDIF tests
	PYTHONPATH=$(SRC_DIR) $(POETRY) run python -c "from flext_tap_ldif.ldif_processor import run_tests; run_tests()"
